...
int main(int argc, char** argv)
{
    ...
    int analytic_func; mi = opts.find("-analytic_function");  assert(mi!=opts.end()); { istringstream ss((*mi).second); ss>>analytic_func; }

    double fit_accuracy; mi = opts.find("-bdsurf_fit_accuracy");  assert(mi!=opts.end()); { istringstream ss((*mi).second); ss>>fit_accuracy; }
    char filepoly[300];	 mi = opts.find("-poly_coeffs_file"); assert(mi!=opts.end()); { istringstream ss((*mi).second); ss>>filepoly; }
    char evaluator_type[300];	 mi = opts.find("-evaluator_type"); assert(mi!=opts.end()); { istringstream ss((*mi).second); ss>>evaluator_type; }
    string eval_type(evaluator_type);
    ifstream tmppoly(filepoly);

    SurfaceEvaluator* evaluator;
    if(eval_type == "blended"){
    ...
    } else if(eval_type == "polynomial"){
        evaluator = new PolynomialEvaluator(string(filepoly), string(filec));
    } else if(eval_type == "analytic"){
        if(analytic_func == 0)
            evaluator = new AnalyticEvaluator(string(filec), &sin3);
        else if(analytic_func == 1)
            evaluator = new AnalyticEvaluator(string(filec), &sin_plus_sin);
        else if(analytic_func == 2)
            evaluator = new AnalyticEvaluator(string(filec), &exp_sin_cos);
        else if(analytic_func == 3)
            evaluator = new AnalyticEvaluator(string(filec), &torus);
        else if(analytic_func == 4)
            evaluator = new AnalyticEvaluator(string(filec), &gaussian);
    } 
    assert(evaluator);
    MPI_Init(NULL,NULL);
    ...
    //face_map->_legacy = false;
    face_map->setup();
    cout <<" num int points: " <<  face_map->gpmesh().get_interior_points().size() << endl;
    MPI_Finalize();
